	...
	Calendar.prototype._update_modal = function() {
		...
		$('a[data-event-id]', this.context).on('click', function(event) {
			...
			if(!modal.data('handled.bootstrap-calendar') || (modal.data('handled.bootstrap-calendar') && modal.data('handled.event-id') != event.id)) {
				...
				modal_body.html('<i class="fa fa-spin fa-spinner"></i> Loading&hellip;');
				...
				modal.off('show.bs.modal').off('hidden.bs.modal').off('shown.bs.modal').on('shown.bs.modal', function() {
	...
	Calendar.prototype._update_month_year = function() {
		...
		$('.cal-month-day, .cal-year-box .span3')
			...
			.on('click', function(event) {
				if($('.events-list', this).length == 0) return;
				if($(this).children('[data-cal-date]').text() == self.activecell) return;
				downbox.show().appendTo(this); //DK 5/58/205 - trying to make it work on touch device. this makes it work, but not sure if it has other repercussions elesewhere.
